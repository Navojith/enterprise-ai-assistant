...
| Containerized backend can't reach Postgres or the MCP server                                                                                              | Compose-network traffic uses service names and _internal_ ports (`postgres:5432`, `mcp_server:8100`), not the host-published ones (`localhost:5433`) — `docker-compose.yml`'s `backend` service overrides `DB_HOST`/`DB_PORT`/`MCP_SERVER_HOST` for exactly this reason. Confirm those overrides are present rather than editing `.env`, which native runs still depend on.                                                                                                                                                                                          |
